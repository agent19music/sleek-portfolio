...
export default function SponsorShowcase({
        ...
        <div className="rounded-lg bg-white dark:bg-zinc-900 p-6 sm:p-8 border border-black/10 dark:border-white/5">
          <div className="flex flex-col items-center justify-center text-center py-8">
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 px-4 sm:px-6 py-2.5 sm:py-3 rounded-lg bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 text-sm sm:text-base font-medium transition-[background-color,transform] duration-150 ease-out hover:bg-zinc-800 dark:hover:bg-white/90 pointer-fine:hover:scale-[1.02] active:scale-[0.98] touch-manipulation"
              style={{
  ...
  const SponsorCard = ({ sponsor }: { sponsor: Sponsor }) => {
    const cardContent = (
      <div className="flex flex-col items-center text-center w-full p-5 sm:p-6 bg-white dark:bg-zinc-900 border border-black/10 dark:border-white/5 rounded-lg transition-[opacity,transform,border-color,box-shadow] duration-300 group-has-[:hover]:opacity-40 group-has-[:hover]:group-hover/item:opacity-100 group-has-[:hover]:group-hover/item:border-black/20 group-has-[:hover]:group-hover/item:dark:border-white/10 group-has-[:hover]:group-hover/item:scale-[1.02] group-has-[:hover]:group-hover/item:shadow-lg group-has-[:hover]:group-hover/item:shadow-black/5 dark:group-has-[:hover]:group-hover/item:shadow-black/20">
        <div className="relative w-16 h-16 sm:w-20 sm:h-20 rounded-full overflow-hidden bg-white dark:bg-white/5 border-2 border-black/10 dark:border-white/10 mb-4 transition-[border-color,box-shadow] duration-300 group-has-[:hover]:group-hover/item:border-black/20 group-has-[:hover]:group-hover/item:dark:border-white/20 group-has-[:hover]:group-hover/item:shadow-md shrink-0">
          <Image
            src={sponsor.logo}
            alt={sponsor.name}
            width={80}
